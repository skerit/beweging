...
let Proc = Fn.inherits('Informer', 'Develry.Beweging', function PythonProcess(parent) {
	...
	this.lightness_calc = new Beweging.Calculator();
...
Proc.setMethod(function processLocs(data) {
	...
	// console.log('');
	// console.log('Processing frame', data.frame, 'with rect count:', data.rects.length);
	...
	// Add the average lightness
	this.lightness_calc.add(~~data.average);

	// Calculate the lightness trend
	// 0 = steady, 1 = lighter, -1 = darker
	trend = this.lightness_calc.getTrend();

	// If there were a few frames of non-motion, assume it's the same event
	if (jitter < 2 && this.previous_stillness < 3 && this.previous_jitter) {
	...
	if ((zero_coordinates > 3 && trend != 0) || zero_coordinates > 4) {
		// The entire frame has changed, that's not right
		jitter -= 3;
	}

	//console.log('Zeroc:', zero_coordinates, jitter, data)

	if (jitter > 1 && jitter < 3 && zero_coordinates > 2) {
		jitter -= 3;
		this.consecutive_jitter = jitter;
	}

	// Accept 2 consecutive frames as movement?
	if ((jitter > 1 && trend == 0) || jitter > 3) {
		...
		//console.log(' -- Motion because jitter ==', jitter, 'And trend:', trend, '. Zero coordinates:', zero_coordinates);
	...
	if (zero_coordinates > 2) {
		if (this.consecutive_jitter) {
			this.consecutive_jitter -= 2;
			this.previous_jitter = 0;
		}

		this.previous_jitter = 0;

		//console.log('Emitting light because', zero_coordinates)
